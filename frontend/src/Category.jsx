      ...
      <div className='row mb-4'>
        {categoryData && categoryData.map((row, index) =>
          <div className='col-md-3 mb-3'>
            <div className="card">
              {row.image && (
                <img src={row.image} className="card-img-top" alt={row.name} style={{ width: "200px", height: "150px", objectFit: "cover" }} />
              )}
              <div className="card-body">
                <h5 className="card-title"><Link to={`/course/category/${row.slug}`}>{row.name}  ({row.total_courses})</Link></h5>
